...
def merge_poster_images(csv_path, posters, personal_urls):
    """Preserve existing CSV entries; add newly-discovered personal URLs."""
    existing = {}
    ...
    with open(csv_path, "w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(
            f, fieldnames=["poster_id", "date", "artist", "personal_url", "official_url"])
        writer.writeheader()
        for r in rows:
            writer.writerow(r)
...
def build(concerts_src, posters_src, out_dir="data", skip_wiki=False, verbose=False):
    ...
    # Merge poster image URLs
    img_rows = merge_poster_images(f"{out_dir}/poster_images.csv", posters, personal_urls)
